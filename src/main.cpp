      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            // Slider control
            client.println("<input type=\"range\" min=\"-150\" max=\"150\" class=\"slider\" id=\"servoSlider\" onchange=\"servo(this.value)\" value=\""+valueString+"\"/>");            
            client.println("<h2 style=\"color:#ffffff;\">Tilt: <span id=\"servoPos2\"></span>&#176;</h2>"); 
            client.println("<input type=\"range\" min=\"-45\" max=\"45\" class=\"slider\" id=\"servoSlider2\" onchange=\"servo2(this.value)\" value=\""+valueString+"\"/>");
            ...
            client.println("$.get(\"/?value=\" +\"X\"+pos + \"&\"); {Connection: close};}</script>");
